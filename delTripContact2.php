...
//put in the database to handle connection
	...
	$query = "delete from tripContact where tripID=$tid" or die("Error in the query");
	//echo($query);
	try{$result=$db->query($query);}catch(Exception $e){echo "There was an error loading data";}
	
	//rollback changes on fail
	if(!$db->commit()){
		$log->LogError("Updating Trip Contact $tid failed... Rolling Back");
		$db->rollback();
	}
	else{
		echo "Trip Contact Updated";
	}
